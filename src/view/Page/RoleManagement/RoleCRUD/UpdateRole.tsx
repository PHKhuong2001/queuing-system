...
import { findRoleById, updateRole } from "@/features/role/roleSlice";
...
function RoleUpdate() {
  const role = useSelector((state: RootState) => state.role.roleUpdate);
  const [formValue, setFormUpdateValue] = useState<IRole>({
    describe: "",
    nameRole: "",
  });
  ...
  useEffect(() => {
    ...
    if (typeof role !== "undefined") {
      setFormUpdateValue(role);
    }
  }, [role]);

  useEffect(() => {
    dispatch(findRoleById(id || ""));
  }, [dispatch, id]);

  const handlerSubmitFormUpdate = () => {
    console.log(formValue);
    dispatch(updateRole({ role: formValue, roleId: id || "" }));
